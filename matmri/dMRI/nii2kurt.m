...
%cos4Thet = (8*cosThet.^4 + 8*cosThet.^2 + 1); % expanded version
...
% (rrios) expanding and heavily simplifying expresion only using cosThet 
% to avoid calculating cos2Thet and cos4Thet that can cause GPU memory full error
A = cat( 2, ones(size(cosThet),'like',cosThet), ...
    -bval.*(1-cosThet.^2), -bval.*cosThet.^2, ...
    bval.^2/6.*(5*cosThet.^4 - 6*cosThet.^2 + 1), ...
    bval.^2/6.*((1/2)*cosThet.^2 .*(5*cosThet.^2 - 3)), ...
    bval.^2/6.*(-15/2*(cosThet.^4 - cosThet.^2)) );
clear cosThet 
